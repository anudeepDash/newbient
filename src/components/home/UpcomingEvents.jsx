...
const EventTicket = ({ event, handleShare }) => {
    return (
        <div id={`event-card-${event.id}`} className="relative bg-zinc-900 border border-white/5 rounded-[3rem] overflow-hidden flex flex-col h-[520px] transition-all duration-500 hover:border-white/10 group">
            {/* Visual Perforations */}
            ...
            <div className="h-[65%] relative overflow-hidden">
                {event.image ? (
                    ...
                    <div className="absolute inset-0 bg-zinc-800 flex items-center justify-center text-gray-600 font-bold uppercase tracking-widest text-xs">
                        TBA
                    </div>
                )}
                {/* Deeper Gradient to hide text in images */}
                <div className="absolute inset-0 bg-gradient-to-t from-zinc-900 via-zinc-900/80 to-transparent" />
                
                <div className="absolute top-6 left-6 px-4 py-2 rounded-xl bg-black/60 backdrop-blur-md border border-white/10">
                    <span className="text-[10px] font-black uppercase tracking-widest text-neon-blue">
                        {event.date ? new Date(event.date).toLocaleDateString('en-US', { day: 'numeric', month: 'short' }) : 'Soon'}
                    </span>
                </div>
            </div>

            {/* Bottom Content Section */}
            <div className="h-[35%] p-8 flex flex-col justify-between relative bg-zinc-900">
                <div>
                ...
                <div className="flex items-center justify-between pt-4">
                    <div className="text-neon-blue font-black tracking-widest flex items-center gap-2 group-hover:gap-4 transition-all">
                        <span className="text-[10px] uppercase">Access Event</span>
                        <ArrowRight size={16} />
                    </div>
                    
                    <button
                        onClick={(e) => handleShare(e, event)}
                        className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-neon-blue hover:text-black transition-all"
                    >
                        <Share2 size={14} />
                    </button>
                </div>
