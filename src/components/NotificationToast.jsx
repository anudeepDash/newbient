...
import { X, Bell, Megaphone, Ticket, MessageSquare, Gift, FileText, Calendar } from 'lucide-react';

const icons = {
    announcement: <Megaphone className="text-neon-blue" size={18} />,
    ticket: <Ticket className="text-neon-pink" size={18} />,
    message: <MessageSquare className="text-neon-green" size={18} />,
    giveaway: <Gift className="text-neon-yellow" size={18} />,
    blog: <FileText className="text-white" size={18} />,
    campaign: <Calendar className="text-neon-purple" size={18} />,
    gig: <Calendar className="text-neon-cyan" size={18} />,
    default: <Bell className="text-gray-400" size={18} />
};
...
const NotificationToast = () => {
    ...
    useEffect(() => {
        ...
        const now = new Date();
        // Look for notifications created in the last 10 seconds
        ...
        <div className="fixed bottom-8 right-4 z-[200] pointer-events-none md:right-8 lg:bottom-12">
            <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div className="w-12 h-12 rounded-2xl bg-black/40 border border-white/5 flex items-center justify-center shrink-0 relative z-10">
                        {activeToast.image ? (
                            <img src={activeToast.image} alt="" className="w-full h-full rounded-2xl object-cover" />
                        ) : (
                            icons[activeToast.type] || icons.default
                        )}
                    </div>
