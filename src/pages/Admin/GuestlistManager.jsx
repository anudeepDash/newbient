            ...
            <div className="relative z-10 max-w-[1400px] mx-auto pb-32">
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    {guestlists && guestlists.length > 0 ? (
                        guestlists.map((gl) => (
                            <Card key={gl.id} className="p-0 bg-zinc-950/40 backdrop-blur-3xl border border-white/5 rounded-[2rem] overflow-hidden group hover:border-neon-blue/30 transition-all duration-700 shadow-2xl flex flex-col h-full">
                                {/* Card Header Media */}
                                <div className="h-48 relative overflow-hidden">
                                    {gl.image ? (
                                        <img src={gl.image} alt={gl.title} className="w-full h-full object-cover opacity-40 group-hover:opacity-70 group-hover:scale-110 transition-all duration-1000" />
                                    ) : (
                                    ...
                                    <div className="absolute top-6 right-6 flex gap-2">
                                        ...
                                        {!gl.guestlistEnabled && (
                                            <div className="px-3 py-1 rounded-full border border-white/10 bg-white/5 text-gray-400 text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5">
                                    ...
                                    <div className="absolute bottom-6 left-8 right-8">
                                        <h3 className="text-2xl font-black italic uppercase tracking-tighter text-white truncate drop-shadow-2xl">{gl.title}</h3>
                                        <div className="flex items-center gap-4 mt-2 opacity-60">
                                            <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                                                <Calendar size={10} className="text-neon-blue" /> {gl.date || 'TBD'}
                                            </span>
                                            <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                                                <MapPin size={10} className="text-neon-blue" /> {gl.location || 'VENUE'}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-8 flex-grow flex flex-col justify-between">
                                    ...
                                    <div className="mt-10 pt-6 border-t border-white/5 grid grid-cols-2 gap-3">
                                        <Button 
                                            variant="outline" 
                                            onClick={() => navigate(`/admin/ticketing?eventId=${gl.id}`)}
                                            className="h-14 rounded-2xl border-white/5 bg-white/5 hover:bg-neon-blue hover:text-black transition-all flex items-center justify-center gap-2 group"
                                        >
                                            <UserCheck size={20} />
                                        ...
                                        <div className="flex gap-2">
                                            <Button 
                                                ...
                                                className={cn(
                                                    ...
                                                    gl.status === 'Open' ? "bg-white/5 text-neon-green hover:bg-neon-green hover:text-black" : "bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white"
                                                )}
                                                title={gl.status === 'Open' ? "Close Guestlist" : "Open Guestlist"}
                                            >
                                                {gl.status === 'Open' ? <Unlock size={22} /> : <Lock size={22} />}
                                            </Button>
                                            <Button variant="outline" onClick={() => handleEdit(gl)} className="flex-1 h-14 rounded-2xl border-white/5 bg-white/5 hover:bg-white hover:text-black transition-all flex items-center justify-center">
                                                <Edit size={20} />
                                            </Button>
                                            <Button 
                                                variant="outline" 
                                                onClick={() => { if(confirm('Purge guestlist vault?')) deleteGuestlist(gl.id); }}
