    ...
    const generatePDF = async () => {
        ...
        const generatePDF = async () => {
            ...
            try {
                ...
                // Use integer scale to prevent text kerning/overlap issues
                const canvas = await html2canvas(invoiceRef.current, {
                    ...
                    onclone: (clonedDoc) => {
                        // Optional: You can force specific styles for printing here if needed
                        // clonedDoc.getElementById('my-element').style.display = 'block';
                    }
                });

                // JPEG is much smaller/faster than PNG for photos/gradients
        ...
        const handleSaveToDB = async (skipPdf = false) => {
            setGenerating(true);
            try {
                alert("DEBUG: Starting Save Process..."); // Debug 1
                ...
                if (!skipPdf) {
                    ...
                    alert("DEBUG: Step 1 - Generating PDF...");
                    const pdf = await generatePDF();

                    if (pdf) {
                        try {
                            alert("DEBUG: PDF generated. Creating blob and uploading...");
                            ...
                            alert("DEBUG: Step 2 - Uploading to Firebase Storage...");
                            const storageRef = ref(storage, `invoices/Newbi_INV-${formData.invoiceNumber}_${Date.now()}.pdf`);
                            await uploadBytes(storageRef, pdfFile);
                            downloadURL = await getDownloadURL(storageRef);
                            console.log("Download URL:", downloadURL);
                            alert("DEBUG: PDF Uploaded. Download URL: " + downloadURL);
                        } catch (uploadError) {
                            console.error("Upload Error:", uploadError);
                            if (!window.confirm("PDF Upload Failed: " + uploadError.message + "\n\nSave data without PDF link?")) {
                                setGenerating(false);
                                return;
                            }
                        }
                    } else {
                        const proceed = window.confirm("PDF Generation Failed visually. \n\nDo you want to SAVE the invoice data anyway? (PDF link will be empty)");
                        if (!proceed) {
                            setGenerating(false);
                            return;
                        }
                    ...
                    alert("DEBUG: Skipping PDF generation.");
                }

                // 3. Sanitize Data
                console.log("Step 3: Sanitizing Data...");
                alert("DEBUG: Step 3 - Sanitizing Data...");
                const cleanItems = items.map(item => {
                    ...
                    return {
                        ...
                        qty: Number(item.qty) || 0, // Corrected from item.quantity to item.qty
                        price: Number(item.price) || 0
                    };
                });
                alert("DEBUG: Data Sanitized.");

                // 4. Save to Firestore
                console.log("Step 4: Saving to Firestore...");
                alert("DEBUG: Step 4 - Preparing data for Firestore...");
                const newInvoice = {
                ...
                alert("DEBUG: Data prepared. Saving to Firestore...");

                await addInvoice(newInvoice);
                console.log("Firestore save complete!");
                alert("DEBUG: Firestore save complete!");
                ...
                if (window.confirm("Invoice Saved Successfully! \n\nClick OK to Create Another Invoice.\nClick Cancel to View All Invoices.")) {
                    ...
                    setItems([{ id: Date.now(), description: '', customValues: {}, qty: 1, price: 0 }]); // Corrected from quantity to qty
                } else {
